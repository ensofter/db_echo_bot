...
# Функция возвращающая безопасную строку conninfo - безопасная, значит все экранируется
...
async def get_pg_connection(
    ...
    conninfo = build_pg_connection(db_name, host, port, user, password)
...
async def get_pg_pool(
    ...
    try:
        db_pool = await AsyncConnectionPool(
            conninfo=conninfo,
            min_size=min_size,
            max_size=max_size,
            timeout=timeout,
        ...
        if db_pool and not db_pool.close:
            await db_pool.close()
        raise
