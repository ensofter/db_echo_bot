...
async def add_user(
    ...
    async with conn.cursor() as cursor:
        await cursor.execute(
            query="""
                INSERT INTO users(user_id, username, languange, role, is_alive, banned)
                VALUES(
                    %(user_id)s,
                    %(username)s,
                    %(language)s,
    ...
    logger.info(
        ...
        "languange='%s', role=%s, is_alive=%s, banned=%s",
        "users",
        user_id,
...
async def change_user_banned_status_by_username(
    ...
    logger.info("Update `banned` status to `%s` for username %s", banned, username)
    ...
    async with conn.cursor() as cursor:
        await cursor.execute(
            query="""
                ...
                WHERE user_id = %s;
            """,
            params=(languange, user_id),
        )
    logger.info("The language `%s` is set for user %d", languange, user_id)
...
async def get_user_lang(
    ...
    async with conn.cursor() as cursor:
        data = await cursor.execute(
            query="""
                SELECT languange FROM users WHERE user_id = %s;
            """,
            params=(user_id,),
        )
        row = await data.fetchone()
    if row:
        logger.info("The user with `user_id`=%s has languange %s", user_id, row[0])
    else:
...
async def get_statistics(
    ...
    logger.info("Users activity got from table=`activity`", len(rows))
